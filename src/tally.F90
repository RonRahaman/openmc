...
!$omp atomic
              t % results(score_index, filter_index) % value = t % results(score_index, filter_index) % value + score
            end do
            j = j + t % scatt_order(j)
            cycle SCORE_LOOP
...
!===============================================================================
  ...
  subroutine score_fission_eout(p, t, i_score)
    ...
    do k = 1, p % n_bank
      ...
      ! Add score to tally
!$omp atomic
      t % results(i_score, i_filter) % value = t % results(i_score, i_filter) % value + score
    end do
        ...
        if (uvw(3) > 0) then
          do j = ijk0(3), ijk1(3) - 1
            ...
            if (all(ijk0 >= 0) .and. all(ijk0 <= m % dimension)) then
              ...
              filter_index = sum((matching_bins(1:t%n_filters) - 1) * t % stride) + 1
!$omp atomic
              t % results(1, filter_index) % value = t % results(1, filter_index) % value + p % wgt
            end if
          end do
        else
